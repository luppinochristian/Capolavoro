...
function initSchema($pdo) {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS notizie (
        ...
        titolo_de VARCHAR(500) DEFAULT NULL,
        testo_de TEXT DEFAULT NULL,
        titolo_es VARCHAR(500) DEFAULT NULL,
        testo_es TEXT DEFAULT NULL,
    ...
    if ($pdo->query("SELECT COUNT(*) as c FROM strutture")->fetch()['c'] == 0) {
        $pdo->exec("INSERT INTO strutture VALUES
            (1,'Campetto Base',50000,1,1,3,'+1 allenamento, +1 crescita, -3% infortuni'),
            (2,'Spogliatoio Attrezzato',110000,2,1,7,'+2 allenamento, +1 crescita, -7% infortuni'),
            (3,'Palestra e Campo',200000,2,2,12,'+2 allenamento, +2 crescita, -12% infortuni'),
            (4,'Centro Sportivo',380000,3,3,18,'+3 allenamento, +3 crescita, -18% infortuni'),
            (5,'Centro High-Tech',600000,4,4,25,'+4 allenamento, +4 crescita, -25% infortuni'),
            (6,'Centro d''Elite',950000,5,5,32,'Staff medico, +5 allenamento, +5 crescita, -32% infortuni'),
            (7,'Academy Personale',1500000,6,6,40,'+6 allenamento, +6 crescita, -40% infortuni')
        ");
    }
    ...
    $migrations = [
        ...
        "ALTER TABLE notizie ADD COLUMN titolo_de VARCHAR(500) DEFAULT NULL",
        "ALTER TABLE notizie ADD COLUMN testo_de TEXT DEFAULT NULL",
        "ALTER TABLE notizie ADD COLUMN titolo_es VARCHAR(500) DEFAULT NULL",
        "ALTER TABLE notizie ADD COLUMN testo_es TEXT DEFAULT NULL",
        "ALTER TABLE players MODIFY COLUMN ai_avatar MEDIUMTEXT DEFAULT NULL",
        // v6: dettaglio tabellino partita (gol/assist/voto del giocatore per la singola partita)
        "ALTER TABLE log_mensile ADD COLUMN gf INT DEFAULT 0",
        "ALTER TABLE log_mensile ADD COLUMN gs INT DEFAULT 0",
        "ALTER TABLE log_mensile ADD COLUMN giornata INT DEFAULT 0",
        "ALTER TABLE log_mensile ADD COLUMN competizione VARCHAR(50) DEFAULT 'lega'",
    ];
    foreach ($migrations as $sql) {
        try { $pdo->exec($sql); } catch(Exception $e) { /* Colonna già esistente */ }
    }

    // ── v6: ribilanciamento valori strutture su DB esistenti ─────────────────
    // Aggiorna i bonus delle strutture già seminate con i nuovi valori bilanciati.
    try {
        $strutture_nuove = [
            [1,1,1,3],  [2,2,1,7],  [3,2,2,12], [4,3,3,18],
            [5,4,4,25], [6,5,5,32], [7,6,6,40],
        ];
        $upd = $pdo->prepare("UPDATE strutture SET bonus_allenamento=?, bonus_crescita=?, riduzione_infortuni=? WHERE livello=?");
        foreach ($strutture_nuove as [$liv,$all,$cre,$inf]) {
            $upd->execute([$all,$cre,$inf,$liv]);
        }
    } catch (Exception $e) { /* tabella non ancora presente: verrà seminata da seedData */ }
